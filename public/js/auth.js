...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // not just on submit via the pattern attribute. Outside the register-form
  // guard because the profile page edits the very same three fields, and a rule
  // that only applies where a name is first typed is no rule at all. Each field
  // is looked up individually, so a page with none of them does nothing.
  ...
  if (orgCascade && orgHidden) {
    ...
    (async function initCascade() {
      // On the profile page the member already belongs somewhere, and the
      // markup says so via data-selected-id. Opening on an empty first step
      // would read as "you have no organization" and invite them to re-pick
      // one they never changed — so the existing branch is rebuilt, every
      // step selected, exactly as the search shortcut does it. Registration
      // has no such attribute and starts empty, as it should.
      const preselect = orgCascade.dataset.selectedId;
      if (preselect) {
        try {
          await fillCascadeFor(preselect);
          return;
        } catch (err) {
          // Fall through to the empty cascade below: a member who cannot see
          // their current organization can at least still choose one.
        }
      }
